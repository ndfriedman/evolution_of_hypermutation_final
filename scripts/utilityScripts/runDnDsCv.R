...
run_dnds_on_normal_and_hypermutated_tumors(cancerTypes){
  ...
  dirPath <- '/Users/friedman/Desktop/mnt/juno/work/taylorlab/friedman/myAdjustedDataFiles/mafsForDndsCV/'
  ...
  for(cancerType in cancerTypes){
    ...
    normalPath <- paste(dirPath, cancerType, '_Normal_DNDS_maf.tsv', sep='')
    hyperPath <- paste(dirPath, cancerType, '_Hypermutated_DNDS_maf.tsv', sep='')
    normalMaf <- read.table(normalPath, sep = '\t', header=TRUE)
    hyperMaf <- read.table(hyperPath, sep = '\t', header=TRUE)
    normalMaf <- normalMaf[,c('sampleID', 'chr' ,'pos' ,'ref' ,'alt', 'gene')]
    hyperMaf <- hyperMaf[,c('sampleID', 'chr' ,'pos' ,'ref' ,'alt', 'gene')]
    ...
    dndsResults[[cancerType]] <- localList
  }
...
cancerTypes=
  c('Colorectal_Cancer', 'Endometrial_Cancer', 'Glioma',
    'Prostate_Cancer', 'Esophagogastric_Cancer', 'Bladder_Cancer')

dndsResults <- run_dnds_on_normal_and_hypermutated_tumors(cancerTypes)
